...
export const Footer = () => {
  return (
    <div className='w-full flex gap-5  h-screen bg-zinc-800 p-20'>
        <div className={`w-1/2 ${foundersGrotesk.className} flex justify-between flex-col h-full `}>
        <div className="heading">

        <h1  className=' text-[8vw] font-semibold uppercase -mb-20 '>
            Eye-
        </h1>
        <h1 className=' text-[8vw] font-semibold uppercase -mb-20 '>
            Oppening
        ...
        <div className='w-1/2'>
         <h1  className={`${foundersGrotesk.className} text-[8vw] font-semibold uppercase -mb-20 `}>
            presentations
        </h1>
        <div className="links flex flex-col gap-3 mt-12">
            {["Facebook" , "InstaGram" ,"Twitter" ,"LinkedIn"].map((item,index)=>(
                <p key={index} className={`${foundersNeue.className}underline text-lg `}>{item}</p>
            ))}
        </div>
        </div>
    </div>
